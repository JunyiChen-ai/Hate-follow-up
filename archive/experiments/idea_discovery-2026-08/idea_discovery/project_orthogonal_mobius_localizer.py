...
from scripts.idea_discovery.audit_less_views import (
    imagebind_text_embeddings, load_predictions, resize, text_curve,
)
...
def calibrated_mobius_field(
    ...
    seed_bytes = hashlib.sha256(streams.astype(np.float32).tobytes()).digest()
    generator = np.random.default_rng(int.from_bytes(seed_bytes[:8], "little"))
    null_maxima = []
