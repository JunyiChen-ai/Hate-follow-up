#!/usr/bin/env python3
"""Compose HVL raw outputs into final frame scores and evaluate (copied from experiments/20260910_spvl/compose.py).
...
def main():
    ...
    ap.add_argument("--intercept", choices=["spvl", "zv_rev", "zv_rev_plus_mean", "legacy", "none", "mean_win", "zv_plus_mean", "lme_zv_top3", "zv_plus_lme", "zv_plus_kmean", "zv_plus_extent", "zv_plus_median", "zv_plus_tophalf"], default="zv_rev")
    ...
    with open(out_path, "w") as fh:
        for r in rows:
            ...
            if a.intercept == "spvl":
                ...
                intercept = 0.0
            elif a.intercept in ("zv_rev", "zv_rev_plus_mean"):
                zr = r["extra"].get("z_rev")
                if zr is None:
                    zr = r["extra"]["z_video"]  # no revision step in this run: fall back to the first verdict
                w = np.array([x["z"] for x in r["extra"]["windows"]], float)
                intercept = float(zr) + (float(w.mean()) if a.intercept == "zv_rev_plus_mean" and len(w) else 0.0)
            else:
                w = np.array([x["z"] for x in r["extra"]["windows"]], float)
