...
ROC .148, i.e. the verdict depends on material outside the window carrying the slur). An absolute
...
Consequence: **pooled ROC and pooled PR cannot change by construction in round 1** (the intercept is
identical and pooled ordering across videos is dominated by it), so any movement is in within, and the
attribution is clean. (`compose.py --residual` gains a source switch; the arithmetic is the same.)
