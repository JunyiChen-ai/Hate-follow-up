#!/usr/bin/env python3
"""NGA: negative-only group adaptation. Label-free, supervised only by the model's own video verdict.

SDL (archived) showed that a bag constraint of the form "one window up, everything else down" is false for
this task: hateful videos have a median positive fraction of .24 / .47, and forcing the constraint
flattened the window curve and destroyed the ordering. The only part of a video-level pseudo label that is
unambiguously true at window level is the negative side -- in a non-hateful video every window is
non-hateful -- so NGA constrains pseudo-negative videos only and never touches a pseudo-positive one.
...
CODE_PATH = "experiments/20260912_nga/nga.py"
...
def main():
    ...
    ap.add_argument("--exp-id", default="20260912_nga")
