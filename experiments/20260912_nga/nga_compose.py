#!/usr/bin/env python3
"""Compose NGA outputs into frame scores and evaluate through the shared evaluator.

final(t) = [z_video + mean_i(a_i)] + centred rank of a_i, i.e. the SPVL-r2 composition unchanged; the only
difference between an NGA run and the frozen baseline is the adapter that produced a_i. No labels here.
"""
    ...
    with open(out_path, "w") as fh:
        for r in rows:
            if r.get("error") or not r.get("score_curve"):
                fh.write(json.dumps({**r, "method": f"{r.get('method', 'nga')}__{tag}"}) + "\n")
                continue
            W = r["extra"]["windows"]
