...
that changed *what is asked* (pairwise, onset, counterfactual, hypothesis chains, speech-act readouts) all
stayed inside the per-window noise, and asking the model itself to carry temporal state in-context lost
.03 within (HVL). The one lever not yet used is *inference over the measurements after they are taken*.
...
(4–12, 12–20, …). Every 4 s cell except the first is covered by one window of each grid, and each window
...
- Observation model: a window covering cells `I` with modality read `y_m` contributes a log-likelihood
  ratio `y_m / s_m` for "some cell in `I` is hateful" against "no cell in `I` is hateful" (interval support,
  noisy-OR); `s_m` is the corpus standard deviation of modality `m`'s reads, computed from the predictions
  only (label-free, transductive). The window's ratio is the max over its modalities (SPVL-r2 semantics:
...
New MLLM calls per video: one more verdict-free measurement pass (grid B): the prefix is re-encoded once
and the window branches are doubled. SPVL-r2 costs about 1.5 s/video on one 5090 with Qwen3-VL-8B; TIL
costs about 3 s/video (two prefix encodings, twice the branches). Inference is milliseconds on CPU.
...
| dwell `D` | 80 s (declared scan: 40, 160; worst reported) |
...
| A1 | A | per-window scaled max | D = 80 s | prior only (the CPU check of 2026-09-22) |
...
| B1 | B | as A1 | D = 80 s | grid B measures as well as grid A |

Decision rule (declared): A4 is the candidate. It passes the promotion gate against SPVL-r2 (rule 8: no
metric down by more than noise on either corpus; at least one metric up ≥ .01 on both). The *mechanism*
claim requires A4 − A3 ≥ .01 within on both corpora (the observation model adds information beyond
averaging + smoothing); if A4 ≈ A3 the shifted grid is merely a second sample and the honest description
is "explicit temporal prior over isolated reads", claimed at that level. A1 − A0 measures the prior alone.

Expected numbers (from the CPU checks below): A1 ≈ .749 / .617 within; A4 is predicted ≥ .76 / .62 if the
interval model adds boundary information. Pooled ROC / PR expected unchanged (intercept unchanged).
