...
CODE_PATH = "experiments/20260912_bnd/bnd.py"
...
BOUNDARY_QUESTION = (
    "Considering the video up to this point, does the content that violates the above rules BEGIN in "
...
END_QUESTION = (
    "Considering the video up to this point, does the content that violates the above rules END with "
    "THIS window? Answer Yes only if such content is present here and absent from the part of the video "
    "that follows; answer No otherwise."
)
...
def boundary_question(i, n, t1, t2, text, kind):
    body = text.strip() if text and text.strip() else "(no speech)"
    core = BOUNDARY_QUESTION if kind == "begin" else END_QUESTION
    return (f"{window_head(i, n, t1, t2)} Transcript in this window: {body}\n"
            f"Look at the frames whose timestamps fall inside this window.\n\n"
            f"{core}\n\nAnswer \"Yes\" or \"No\".")
    ...
    for i, ((t1, t2), txt) in enumerate(zip(wins, wtexts)):
        ...
        if args.boundary:
            for kind in ("begin", "end"):
                q = boundary_question(i, n, t1, t2, txt, kind)
                bids, _ = judge.branch_ids(msgs, q, history, head_text=head)
                rec[f"b_{kind}"] = judge.cached_margin(cache, bids)
        if args.acts:
...
def main():
    ...
    ap.add_argument("--exp-id", default="20260912_bnd")
    ...
    ap.add_argument("--boundary", type=int, default=1,
                    help="1 adds two boundary reads per window: does the violating content BEGIN here, "
                         "does it END here")
    ...
    args.method_name = args.method_name or (f"bnd_f{args.frames}_w{args.window_seconds:g}"
                                            f"_b{args.boundary}_topic{args.topic}")
    cfg = dict(vars(args))
    cfg.update({"code_path": CODE_PATH, "date": time.strftime("%Y-%m-%d"), "host": socket.gethostname(),
                ...
                "boundary_question": BOUNDARY_QUESTION, "end_question": END_QUESTION,
