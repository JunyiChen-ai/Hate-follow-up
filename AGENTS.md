...
本文件是 `CLAUDE.md` 的 agent 通用版（2026-09-11 由 CLAUDE.md 复制并改写，两份同步维护；规则以 CLAUDE.md 为准，冲突时改这份）。面向任何在本仓库工作的 coding agent（Codex、Claude Code、其它）。
...
- 监控：后台 `until` 循环经 ssh 查 `squeue -j <id>` 消失，再看 `slurm_<id>.out` 尾部有无 `Traceback` / `FAILED` / DONE。
