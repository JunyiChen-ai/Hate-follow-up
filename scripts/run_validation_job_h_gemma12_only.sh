...
#SBATCH --mem=48G
#SBATCH -t 72:00:00
#SBATCH -o /data/jehc223/EMNLP3/logs/validation_job_h_%j.out
#SBATCH -e /data/jehc223/EMNLP3/logs/validation_job_h_%j.err
...
run_judge() {
  local ds="$1"
  local ih_arg=()
  local stem="offline_validation_gemma-3-12b-it"
  if [[ "$ds" == "ImpliHateVid" ]]; then
    ih_arg=(--ih-prompt)
    stem="offline_validation_ih_gemma-3-12b-it"
  fi
  run_supervised_adaptive "$ds" \
    "results/boundary_rescue/${ds}/${stem}.jsonl" \
    "results/validation_runs/crash_skip/judge_gemma-3-12b-it_${ds}.txt" \
    "results/validation_runs/logs/judge_gemma-3-12b-it_${ds}" \
    "16,8,4,2,1" \
...
echo "=== gated MLLM validation: Gemma-3-12B only ==="
for ds in "${DATASETS[@]}"; do
  run_judge "$ds"
done

python scripts/validation_eval_summary.py || true
echo "DONE validation job H"
